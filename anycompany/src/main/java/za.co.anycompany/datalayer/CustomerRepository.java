...
public class CustomerRepository {
        ...
        public static Customer load(int customerId, Order order) {
            Connection con = getDBConnection();
            PreparedStatement prpstmt = null;
            ResultSet resultSet = null;
            Customer customer = new Customer();
            try {
                 int fkCustomerId=order.getCustomer().getCustomerId();
                prpstmt = con.prepareStatement("select * from CUSTOMER where customerId = ?" +
                        "INNER JOIN Customers ON "+fkCustomerId+"=Customer.customerId;");
